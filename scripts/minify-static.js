...
async function minifyFile(filePath) {
    ...
    if (!ALLOWED_TEXT_EXT.includes(ext)) return { skipped: true, reason: 'unsupported-ext' };
    const base = path.basename(filePath).toLowerCase();
    if (base.includes('.min.') || base.endsWith('.min' + ext)) return { skipped: true, reason: 'already-minified' };
    try {
        console.log('Minifying:', filePath);
        let minified;

        if (base.endsWith(".js")) minified = UglifyJS.minify(fs.readFileSync(filePath, 'utf8')).code;
...
(async function main() {
    ...
    for (const f of files) {
        ...
        if (res.error) {
            ...
            errors.push({ file: f, error: res.error.message || res.error.toString() });
            appendLog(`ERROR ${f}: ${res.error && res.error.message ? res.error.message : res.error}`);
        } else if (res.skipped) {
            skippedCount++;
